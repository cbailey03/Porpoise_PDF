...
/// The source pages whose textures are worth keeping.
///
...
/// texture instead of re-rendering. `grid` is the source pages the thumbnail grid is
/// showing, already resolved — it is virtualized, so that is the rows on screen and not the
/// whole document.
///
/// `source_of` maps a display position to a source page. A function rather than a
/// `PageOrder` so this stays pure and the position-versus-page distinction can be tested
/// with an order that actually differs from the identity.
///
...
pub(crate) fn pages_to_keep(
    ...
    margin: usize,
    grid: Vec<usize>,
    source_of: impl Fn(usize) -> Option<usize>,
) -> Vec<usize> {
    let mut keep = grid;
...
mod tests {
    ...
    fn unedited(pages: usize) -> impl Fn(usize) -> Option<usize> {
        move |position| (position < pages).then_some(position)
    }
    ...
    fn a_page_the_grid_is_showing_survives_a_distant_viewport() {
        ...
        let grid: Vec<usize> = (0..12).collect();
    ...
    fn the_column_keeps_a_margin_either_side_of_what_it_shows() {
        ...
        for page in 17..25 {
            assert!(keep.contains(&page), "page {page} missing from {keep:?}");
        }
        assert!(!keep.contains(&16), "kept too far above: {keep:?}");
        assert!(!keep.contains(&25), "kept too far below: {keep:?}");
    }
    ...
    fn positions_are_resolved_through_the_order_before_being_kept() {
        ...
        let reversed = |position: usize| (position < 12).then(|| 11 - position);
        let keep = pages_to_keep(&(0..1), 2, Vec::new(), reversed);
        assert_eq!(keep, vec![11, 10, 9]);
    }
