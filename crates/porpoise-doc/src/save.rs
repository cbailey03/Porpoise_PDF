...
//! the page count for all three.
//!
//! A nested page tree is flattened rather than refused. See [`crate::inherit`] for
//! what has to happen to the pages first.
...
pub fn save_reordered(
    ...
    // Flattening the tree below is what makes a reorder expressible at all, but it
    // also throws away every attribute a page was inheriting from a branch. Write
    // those onto the pages first so the flatten cannot change how any page renders.
    inherit::push_down(&mut document, root).map_err(|error| SaveError::PageTree {
        path: source.to_path_buf(),
        detail: error.to_string(),
    })?;
